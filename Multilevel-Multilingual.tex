...
\KeywordTok{estat}\NormalTok{ icc }\CommentTok{// calculate icc}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{performance}\SpecialCharTok{::}\FunctionTok{icc}\NormalTok{(fit0) }\CommentTok{\# calculate icc}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{m0 }\OperatorTok{=} \FunctionTok{fit}\NormalTok{(MixedModel, }\PreprocessorTok{@formula}\NormalTok{(outcome }\OperatorTok{\textasciitilde{}}\NormalTok{ (}\FloatTok{1} \OperatorTok{|}\NormalTok{ country)), df)}
\end{Highlighting}
\end{Shaded}
...
\subsection{Load The Needed Packages And Load The
