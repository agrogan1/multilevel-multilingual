...
In many instances, we may wish to aggregate data. For example, we may wish to create variables representing the average level of an indicator across a group. Aggregating data can also be useful in creating group level means, and is an important part of the correlated random effects model.

In the examples below, I create a group level variable for `warmth`, representing the average level of warmth in each group. If warmth is denoted by $\text{warmth}_{ij}$ then the group level variable is denoted by $\overline{warmth}_{.j}$.
