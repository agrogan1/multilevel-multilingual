...
However, spreadsheets may be problematic as a data storage solution for a number of reasons detailed below, especially as data sets grow in size. Notably, statistical programs like [Stata](https://www.stata.com/), [R](https://www.r-project.org/), or [Julia](https://julialang.org/) can all store additional information with each variable such as: a *variable label*, describing the contents of the variable, or the survey question that resulted in the variable; and a *value label*, which attaches qualitative information to each possible value of the response. 
...
Notice how a description of the data no long er contains much of the information that helped us to understand the variables.
...
Secondly, a spreadsheet would allow some of your columns to have the same name, which might make data difficult to work with in other software.
